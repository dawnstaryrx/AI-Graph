...
(function ($) {
    ...
    $.fn.extend({
         ...
         * 卸载模块
         */
        uninstall: function (index, url, msg) {
            $(this).click(function () {
                var param = {};
                param[index] = $(this).attr('data-id');
                layer.confirm(msg ? msg : '确定要卸载吗？', {title: '友情提示'}
                    , function (index) {
                        $.post(url, param, function (result) {
                            result.code === 1 ? $.show_success(result.msg, result.url)
                                : $.show_error(result.msg);
         ...
         * @param option
         */
        tpl_form_field_image: function (option) {
            var $this = this
                // 配置项
                , defaults = {
                    name: 'iFile'            // input name
            ...
            $this.fileLibrary({
                ...
                , done: function (data, $touch) {
                    ...
                    var $thisIput = $("input[name="+name+"]");
                    $thisIput.val(list[0]['file_name']);
                    $thisIput.parent().next().children('img').attr('src',list[0]['file_path']);
                }
            });
        },
        /**
         ...
         * @param option
         */
        tpl_form_field_multi_image: function (option) {
            var $this = this
                // 配置项
                , defaults = {
                    name: 'iFile'            // input name
            ...
            $this.fileLibrary({
                ...
                , done: function (data, $touch) {
                    ...
                    var name = option.name;
                    var $thisIput = $(".btns-"+name+"");
                    console.log($thisIput.parent().parent().next());
                    $.each(list, function(idx, url){
                    	$thisIput.parent().parent().next().append('<div class="multi-item"><img onerror="this.src="/resource/images/nopic.jpg"; this.title="图片未找到" src="'+url.file_path+'" class="img-responsive img-thumbnail"><input type="hidden" name="'+name+'[]" value="'+url.file_name+'"><em class="close" title="删除这张图片" onclick="deleteMultiImage(this)">×</em></div>');
        			});
         ...
         * @param option
         */
        tpl_form_field_video: function (option) {
            var $this = this
                // 配置项
                , defaults = {
                    name: 'iFile'            // input name
            ...
            $this.fileLibrary({
                ...
                , done: function (data, $touch) {
                    ...
                    var name = option.name;
                    var $thisIput = $("input[name="+name+"]");
                    $thisIput.val(list[0]['file_path']);
                    console.log($thisIput.parent().next().children('video'));
                    $thisIput.parent().next().children('video').attr('src',list[0]['file_path']);
                    $thisIput.parent().next().children('video').attr('data-src',list[0]['file_path']);
                }
            });
        },
        /**
