 var obj;
 var element
//var STORE_URL='http://'+window.location.host
        ...
        moveFilesEvent: function () {
            ...
            $groupSelect.on('click', '.move-file-group', function () {
                ...
                layer.confirm('确定移动选中的文件吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/index.php?s=/store/upload.library/moveFiles', {
                        group_id: groupId
                        , fileIds: fileIds
                    }, function (result) {
        ...
        deleteFilesEvent: function () {
            ...
            _this.$element.on('click', '.file-delete', function () {
                ...
                layer.confirm('确定删除选中的文件吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/index.php?s=/store/upload.library/deleteFiles', {
                        fileIds: fileIds
                    }, function (result) {
                        layer.close(load);
                        if (result.code === 1) {
            ...
            if(_this.options.type=='video'){
            	...
            	var uploader = WebUploader.create({
            	 ...
            	 var uploader = WebUploader.create({
                     ...
                     auto: true,
                     // 文件接收服务端。
                     server: STORE_URL + '/index.php?s=/store/upload.library/uploadFiles',
                     // 选择文件的按钮。可选。
                     // 内部根据当前运行是创建，可能是input元素，也可能是flash.
                     pick: {
                         id: '.j-upload',
...
setTimeout(function(){
	 ...
	 obj.renderFileList();
        ...
        addGroupEvent: function () {
            ...
            _this.$element.on('click', '.group-add', function () {
                layer.prompt({title: '请输入新分组名称'}, function (value, index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/index.php?s=/store/upload.library/addGroup', {
                        group_name: value,
        ...
        editGroupEvent: function () {
            ...
            _this.$element.find('.file-group').on('click', '.group-edit', function () {
                ...
                layer.prompt({title: '修改分组名称', value: $li.attr('title')}, function (value, index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/index.php?s=/store/upload.library/editGroup', {
                        group_id: group_id
                        , group_name: value
                    }, function (result) {
                        layer.msg(result.msg);
        ...
        deleteGroupEvent: function () {
            ...
            _this.$element.find('.file-group').on('click', '.group-delete', function () {
                ...
                layer.confirm('确定删除该分组吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(STORE_URL + '/index.php?s=/store/upload.library/deleteGroup', {
                        group_id: group_id
                    }, function (result) {
                        layer.msg(result.msg);
                        if (result.code === 1) {
        ...
        getJsonData: function (params, success) {
            ...
            $.getJSON(STORE_URL + '/index.php?s=/store/upload.library/fileList', params, function (result) {
                layer.close(loadIndex);
                if (result.code === 1) {
        ...
        getCurrentGroupId: function () {
            return obj.$element.find('.file-group > ul > li.active').data('group-id');
        },
