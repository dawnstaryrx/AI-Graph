...
<li  class="active"><a href="<?php  echo $feedback_link;?>" >学徒/入驻(<?php  echo $feedback_num?>)</a></li>
...
<td><?php  echo $itme['wxid'];?></td>
<td><?php  echo $itme['phone'];?></td>
<td><?php  if($itme['shopid']==0) { ?>入驻申请<?php  } else { ?>拜师(<?php  echo $itme['shopname'];?>)<?php  } ?></td>
<td><?php  if($itme['read']==0) { ?>未处理<?php  } else { ?>已处理<?php  } ?></td>
...
<button type="button" class="btn btn-primary" style="margin:30px auto;margin-top:0px;;" onClick="read()">批量已读</button>
	<?php  echo $pager;?>
	</div>
</div>
...
console.log(timeArr);
		...
		require(['../../../addons/xz_bq_dy/template/libs/com.js?r=<?php  echo mt_rand(1000,1000000)?>'])
function edit(id){
...
function del(){
	...
	function delfun(){
		...
		$.post("<?php  echo $this->createWebUrl('delfeedback')?>", {
...
function read(){
	...
	function delfun(){
		...
		$.post("<?php  echo $this->createWebUrl('delread')?>", {
