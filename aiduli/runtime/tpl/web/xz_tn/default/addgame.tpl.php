...
<div class="form-group"  >
        <label for="firstname" class="col-sm-2 control-label">关于</label>
        <div class="col-sm-10">
...
<div class="form-group"  >
        <label for="firstname" class="col-sm-2 control-label">提示</label>
        <div class="col-sm-10">
...
<div class="form-group"  >
	 ...
	 <div class="form-group"  >
        <label for="firstname" class="col-sm-2 control-label">关卡图片</label>
        <div class="col-sm-10">
          <div class="mui-input-cell"> <?php  echo tpl_form_field_image('imgpathb',$imgpathb,$imgpathb_view)?> </div>
		</div>
      </div>
...
var db_var=["title","tips","idd","about"]
...
db_obj[db_img[i]]=$('input[name="'+db_img[i]+'"]').val()
		...
		$.post("<?php  echo $this->createWebUrl('Addfindgame')?>&type=save&classname=<?php  echo $_GPC['classname'];?>", db_obj,
				function (data) {
			
				 obj = JSON.parse(data);
				
				 if(obj['res']==1){
alertf('操作成功','提醒',function(){window.location.href="<?php  echo $this->createWebUrl('find')?>" })
				 }else{
